#!/usr/bin/env node
/* build-search-index.mjs — regenerate src/data/search-index.json
 *
 * Reads the site's own manifests and markdown, builds one searchable record
 * per thing a visitor might ask "where is ...?" about, and precomputes a
 * 384-dim embedding for each with Ternlight mini.
 *
 * The vectors are committed, so the browser only ever has to embed the
 * visitor's *query* — not the whole corpus. Regenerate whenever you add a
 * staff member, product, service, page, blog or research entry:
 *
 *     npm install --no-save @ternlight/mini
 *     node scripts/build-search-index.mjs
 *
 * Run it with the same Ternlight tier used at runtime (see MODEL below).
 */
...
/* Read a manifest and return its entry list. */
...
/* Pull `**Key:** value` off a markdown line. */
...
for (const entry of manifest('src/staff/manifest.json')) {
  ...
  entities.push({
    ...
    // person-only aliases: name parts + nickname + github. Team names are
    // deliberately excluded so a query like "who does starsec" does not
    // lexically match every StarSec team member.
  ...
  /* each member's blogs / products / research are separately findable */
...
/* Keep the vector text inside the model's 128-token window; over-long
   records get truncated anyway, but this keeps the stored text readable. */
...
const out = {
  ...
  /* entity fields are kept in parallel arrays to keep the file small */
  entities: entities.map(e => ({
    k: e.kind, t: e.title, h: e.href, a: e.answer, al: e.aliases,
    // omit empty optional fields to keep the file small
