...
(function () {
  ...
  var RULES = [
    ...
    { re: /\b(sign ?up|signing|register|registration|compete|competing|competition|comp|tournament|showcase|train(ing)?|rowdy|cyber ?con|cybercon|cyber jedis|jedis|captain|operator|nov(ember)? ?6|nov(ember)? ?7|nov(ember)? ?6.?7)\b/i,
      say: "StarSec is running a cyber competition showcase at Rowdy CyberCon, November 6-7. Friday the 6th is team training and Saturday the 7th is the competition, and both days are required. It is at the Weston Conference Center on the UTSA Downtown Campus (SP1 / San Pedro 1). The signup form is short: name, the role you want (Captain or Operator), experience level, and your details. It lands in a real inbox.",
    ...
    { re: /\b(suggest|suggestion|feedback|idea|ideas|improve|improvement|feature request|request)\b/i,
      say: "Suggestions are genuinely welcome, and pre-filling them with the page you were on helps a lot.",
      action: 'suggest' },
  ...
  function init() {
    if (document.querySelector('.d5dog')) return;
    build();
  }
